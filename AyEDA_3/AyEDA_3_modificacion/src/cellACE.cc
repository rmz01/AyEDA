 ...
 * @param os El flujo de salida en el que se imprimirá la celda.
 */
void CellACEM5::display(std::ostream& os) {
  if (state_ == '0') {
    os << "-";
  } else {
    os << "X";
...
/**
 * @brief Calcula el estado siguiente de la celda ACEM5 basándose en que la mayoria es el valor del estado siguiente
 * 
 * @param reticulo El retículo que contiene el estado actual de todas las celdas.
 */
void CellACEM5::nextState(const Lattice& reticulo) {
  const State C = state_;
  State next_state{0};
  ...
  Position* Left = new PositionDim<1>(1, position - 1);
  Position* Right = new PositionDim<1>(1, position + 1);
  Position* Left_2 = new PositionDim<1>(1, position - 2);
  Position* Right_2 = new PositionDim<1>(1, position + 2);

  State L = reticulo[*Left].getState();
  State R = reticulo[*Right].getState();
  State L2 = reticulo[*Left_2].getState();
  State R2 = reticulo[*Right_2].getState();

  // Next state es la mayoria de sus vecinos y el mismo

  next_state = (L == '1' ? 1 : 0) + (L2 == '1' ? 1 : 0) + (C == '1' ? 1 : 0) + (R == '1' ? 1 : 0) + (R2 == '1' ? 1 : 0) >= 3 ? '1' : '0';
